...
```mermaid
flowchart LR
    Main[Main agent<br/>tier-routed: Haiku/Sonnet/Opus]
...
8. **Subagents + smart routing** — researcher (Haiku 4.5, web search/fetch + grep) and code-reviewer (Sonnet 4.6, read-only file tools). Pre-query Haiku classifier in `index.ts` (and `server.ts`) swaps the main `query()` model between Haiku/Sonnet/Opus per turn before the call. The originally-planned `opus-worker` subagent was dropped — model-swap routing already engages Opus end-to-end when warranted, so a separate Opus subagent would be redundant.
...
- **2026-05-08 — `task_logs` shape: one row per turn, multi-model breakdown in JSONB.** Spec ER diagram (line 206) implies one row per `model_id`; implementation persists one row per turn with `model_usage JSONB` carrying the per-model breakdown. A single `model_id` column would be wrong on subagent turns where main + subagent models both contribute usage. Top-level `input_tokens` / `output_tokens` / `cache_*_tokens` columns are pre-summed across `model_usage` entries so cache-hit-ratio queries don't need `jsonb_array_elements`.
- **2026-05-08 — `task_logs.thread_id` is nullable.** CLI turns also write rows but aren't threaded. FK cascade still applies for non-NULL values.
- **2026-05-08 — `task_logs` adds columns** beyond the spec list: `tier`, `tool_error_count`, `classifier_fallback`, `classifier_cost_usd`, `total_cost_usd`, `source`, `num_turns`. All directly serve the cost-sheet metrics in spec "Observability" lines 245–249 plus step 8's tier-distribution + classifier-fallback rates.
- **2026-05-08 — `/api/cost` exempt from `x-user-id`.** VPS middleware now skips the user-id check when the request path is `/api/cost` (single-user product; cost endpoint serves system-wide aggregates, not user-scoped data). Worker proxy omits the header. Pattern scales to future system-data routes via a path list rather than placeholder values.
- **2026-05-08 — `subagent_cost_share` metric deferred to 7b.** Spec line 248 lists subagent cost share as a cost-sheet metric; precise derivation needs per-tool-call cost which the SDK's `modelUsage` doesn't expose. Ship `subagent_count_total` in 7a; add cost share when 7b's UI clarifies the display semantics.
